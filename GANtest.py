...
Author : solaris33
Project URL : http://solarisailab.com/archives/2483
'''
...
#생성된 MNIST 이미지를 8x8 Grid 로 보여주는 Plot 함수를 정의합니다.
def plot(samples):
...
#생성자를 선언합니다(generator)
G = build_generator(z)

#구분자를 선언합니다(discriminator)
...
d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = \
                D_real_logits, labels = tf.ones_like(D_real_logits)))


d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = \
                D_fake_logits, labels = tf.zeros_like(D_fake_logits)))
...
with tf.Session() as sess:
    ...
    for i in range(num_epoch):
        ...
        if i% 100 ==0:
            print('반복(Epoch): %d, Generator 손실함수(g_loss): %f, Discriminator \
            손실함수(d_loss): %f' % (i, g_loss_print, d_loss_print)).encode('utf-8')
